...
const DataVisualization: React.FC<DataVisualizationProps> = ({ data }) => {
  ...
  console.log('🎨 Renderizando DataVisualization com dados:', {
    activities: stravaActivities.length,
    events: calendarEvents.length,
    insights: insights.length
  })

  // Dados reais para gráficos baseados nas atividades do Strava
  const weeklyActivityData = React.useMemo(() => {
    if (!stravaActivities.length) return []

    return stravaActivities.slice(0, 7).reverse().map((activity, index) => ({
      day: `${index + 1}`,
  ...
  // Dados de progresso real sem simulação
  const progressData = [
    { name: 'Aderência', value: metrics.adherenceScore, color: '#22c55e' },
    { name: 'Consistência', value: metrics.consistencyScore, color: '#3b82f6' },
    { name: 'Melhoria', value: Math.max(0, metrics.improvementRate), color: '#8b5cf6' }
  ]

  // Dados de eventos semanais reais
  const weeklyEventsData = React.useMemo(() => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {metrics.weeklyProgress.averageSpeed.toFixed(1)}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">
                {Math.round(metrics.weeklyProgress.totalCalories)}
              </div>
                ...
                <div key={event.id} className={`p-3 rounded-lg border ${getEventColor(event.type)}`}>
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    <Badge variant="outline" className={`text-xs ${
                      ...
                      event.status === 'missed' ? 'bg-red-50 text-red-700' :
      ...
      {/* Gráfico de Progresso das Atividades Reais */}
          ...
          {weeklyActivityData.length > 0 ? (
            ...
            <div className="h-72 flex items-center justify-center text-gray-500">
              <div className="text-center">
                ...
                <p>Conecte sua conta Strava para ver as atividades aqui</p>
              </div>
            </div>
          )}
        </CardContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {metrics.adherenceScore >= 80 ? 'Excelente!' : 
                 metrics.adherenceScore >= 60 ? 'Bom progresso' : 'Pode melhorar'}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Baseado na regularidade dos treinos
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Comparado com atividades anteriores
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {/* Agenda da Semana Real */}
      {weeklyEventsData.length > 0 && (
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
            ...
            {insights.length > 0 ? (
              ...
              <div className="text-center py-4">
                ...
                <p className="text-sm text-gray-500">
                  Insights aparecerão conforme você usar o TrainerAI
                </p>
              </div>
            )}
          </div>
      ...
      {/* Próximas Metas Reais */}
      {metrics.nextGoals.length > 0 && (
        <Card className="xl:col-span-3">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Target className="w-5 h-5 text-emerald-500" />
